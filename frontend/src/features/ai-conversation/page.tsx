...
export default function AIConversationPage() {
  ...
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState("");
  const [isJyosthnaMode, setIsJyosthnaMode] = useState(false);
...
const handleSubmit = async (
  ...
  const mentionsJyosthna =
    input.toLowerCase().includes("jyosthna");

  if (mentionsJyosthna) {
    setIsJyosthnaMode(true);
  }
  ...
  try {
    const response = await sendConversationMessage(
      updatedMessages,
      isJyosthnaMode || mentionsJyosthna
    );
...
const handleClearConversation = () => {
  ...
  className={
    isJyosthnaMode
      ? "w-full max-w-4xl rounded-2xl border border-pink-100 bg-pink-50/40 p-5 text-slate-800 shadow-sm"
      : "w-full max-w-4xl text-slate-800"
  }
>
                        <div className="mb-2 flex items-center gap-2">
  <div
    className={
      isJyosthnaMode
        ? "flex h-7 w-7 items-center justify-center rounded-lg bg-pink-100 text-sm"
        : "flex h-7 w-7 items-center justify-center rounded-lg bg-slate-900 text-[10px] font-semibold text-white"
    }
  >
    {isJyosthnaMode ? "💗" : "AI"}
  </div>
  ...
  >
    {isJyosthnaMode ? "✨ Special Conversation" : "Assistant"}
  </span>
